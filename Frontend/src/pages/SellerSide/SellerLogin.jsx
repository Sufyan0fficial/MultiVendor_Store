...
import { Seller_Login } from '../../api/routes';
import { storeSellerData } from '../../Redux/SellerSlice';
...
function SellerLogin() {
    ...
    useEffect(() => {
        if (sellerData?._id) {
            Message(messageApi, 'success', 'Seller already logged in')
            setTimeout(() => {
                navigate('/dashboard')
    ...
    const handleLogin = async (values) => {
        ...
        try {
            const res = await Seller_Login(data)
            if (res.status === 200) {
                Message(messageApi, 'success', 'Login Successful')
                dispatch(storeSellerData(res.data?.data))
                setTimeout(() => {
                    ...
                    >
                        ...
                        <div>
                            ...
                            <div className='flex items-center gap-2 mt-6'>
                                ...
                                <span className='text-blue-600 cursor-pointer' onClick={() => navigate('/seller-signup')}>Sign Up</span>
                            </div>
                        </div>
                    </Form>
                </div>
